...
const { Configuracion, Parametro, Respuesta } = require('../models');
...
module.exports = {
  listar: async (req, res) => {
    try {
      const respuesta = await listar(Configuracion.db, {}, {}, {});
  ...
  crear: async (req, res) => {
    try {
      ...
      if (!parametro) {
        throw new Error('No se puedo encontrar el parametro de configuración requerido.');
      }
  ...
  resolver: async (req, res) => {
    try {
      ...
      let parametro = await Parametro.db.findOne({ tipo: 'parametro' });
      if (!parametro) {
        throw new Error('No se puedo encontrar el parametro requerido.');
      }
